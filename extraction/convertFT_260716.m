...
for sub  = 1:length(R.subnames)
    ...
    [trn trdets trstim trdef] = stimex_trialselect(R.subnames{sub});
%       [trn trdets] = stimex_trialselect(R.subnames{sub});
    ...
    for n = 1:length(trn)
        ...
        D = spm_eeg_load([R.datapath '\' parts{4} '\' parts{5} '\SPMstim\fjaspmeeg_uespmeeg_' parts{7}]);
...
%         5;
        for i = 1:2
            % Split into stimulators
            cfg = [];
            cfg.toilim = trdef{1}(n,2*i-1:2*i);
            sFTdata = ft_redefinetrial(cfg, FTdata);
            sFTdata.stimfreq = trstim{n}(i);
            sFTdata.stimside = trdets{n};
            % Downsample
            cfg = [];
            cfg.resamplefs = R.ds;
            cfg.detrend  = 'no';
            sFTdata = ft_resampledata(cfg, sFTdata)
            save([R.analysispath R.pipestamp '\data\split\' R.subnames{sub} '_OFFdrug_' R.pipestamp '_stim' num2str(sFTdata.stimfreq) 'Hz.mat'],'sFTdata')
        end
    end
end
end