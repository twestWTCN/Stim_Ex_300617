...
for f = 1:numel(files)
    ...
    if details.brainamp
     ...
     %% ERROR
     % Error using ft_datatype_sens (line 467)
     % converting to version upcoming is not supported
     % TW
     
     origsens = spm_cat_struct(origsens,  ft_datatype_sens(ft_read_sens(S.dataset, 'senstype', 'meg'), 'version', 'upcoming', 'amplitude', 'T', 'distance', 'mm'));
    ...
    if details.oxford
        lblorig = {};
        lblnew  = {};
    elseif ~isempty(details.montage)
        lblorig = details.montage.lblorig;
        lblnew = details.montage.lblnew;
    else
...
% 
% Error in dbs_meg_stim_prepare_spm12_TW (line 334)
%     pos = cat(3, pos, fid(i).fid.pos);
% 
% Error in extract_continuous_loop_210616 (line 4)
%         dbs_meg_stim_prepare_spm12_TW(R.subnames{sub},drug
% pos = [];
for i = 1:numel(fid)
    pos = cat(3, pos, fid(i).fid.pos);
end
...
if ~isempty(intersect(cl, D.condlist))
    D = condlist(D, cl);
end
