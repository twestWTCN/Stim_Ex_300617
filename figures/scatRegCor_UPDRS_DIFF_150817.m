...
rows = a.cond==R.stimfreq(1) | a.cond==R.stimfreq(2); % & a.contact == 2;
vars = {'subject','cond','side','DFA','PEB','Tot_off','trem_off','tot_on','trem_on'};
T3 = a(rows,vars);
for i = 1:length(vartest)
    Y = []; X = [];
    for side = 1:2
        rows_1 = T3.side == side & T3.cond==R.stimfreq(1);       
        rows_2 = T3.side == side & T3.cond==R.stimfreq(2);
        T1 = T3(rows_1,:); T2 = T3(rows_2,:);
        T4 = T1.DFA - T2.DFA
